...
# Copyright (c) 2018, The Fraenkel Lab at MIT
# All rights reserved.
...
#JFG from axial import axial
# axial 0.2.3  Interactive Visualizations for High Dimensional Genomic Data

logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)
handler = logging.StreamHandler()
handler.setLevel(logging.INFO)
handler.setFormatter(logging.Formatter('%(asctime)s - OI3: %(levelname)s - %(message)s', "%I:%M:%S"))
logger.addHandler(handler)

#JFG moved out of this code
...
class OIgraph:
    ...
    def set_interactome(self, interactome_fp, params=dict()):
        """
        Builds a representation of a graph from an interactome file.

        From the interactome_fp, populates
        ...
        Arguments:
            interactome_fp (str or FILE): tab-delimited text file containing edges in interactome and their weights formatted like "ProteinA\tProteinB\tCost"
            params (dict): params with which to run the program
        """


        self.interactome_dataframe = pd.read_csv(interactome_fp)

          
        acns = [cn for cn in self.interactome_dataframe.columns if cn not in ['nodeName1','nodeName2']]
        self.interactome_graph = nx.from_pandas_edgelist(self.interactome_dataframe, 'nodeName1', 'nodeName2', edge_attr=acns)
        # self.interactome_graph = nx.from_pandas_edgelist(self.interactome_dataframe, 'nodeName1', 'nodeName2', edge_attr=self.interactome_dataframe.columns[2:].tolist())

        # Convert the interactome dataframe from string interactor IDs to integer interactor IDs.
        # Do so by selecting the nodeName1 and nodeName2 columns from the interactome dataframe,
        ...
        (self.edges, self.nodes) = pd.factorize(self.interactome_dataframe[["nodeName1","nodeName2"]].unstack())
        # Here we do the inverse operation of "unstack" above, which gives us an interpretable edges datastructure
        self.edges = self.edges.reshape(self.interactome_dataframe[["nodeName1","nodeName2"]].shape, order='F')
    ...
    def _reset_hyperparameters(self, params=dict()):
        ...
        defaults = {"w": 1, "b": 16, "g": 5, "r": 0.2, "edge_noise": 0.1, "dummy_mode": "terminals", "seed": 123, "skip_checks": False}
        ...
        if hasattr(self, "bare_prizes"): 
            self.prizes = self.bare_prizes * self.params.b
            self.prizes[np.where(self.node_attributes["species"] == 'R')[0]] *= self.params.r
            self.prizes[np.where(self.node_attributes["species"] == 'D')[0]] *= self.params.r
    ...
    def _check_validity_of_hyperparameters(self):
        ...
        if not (isinstance(self.params.r, numbers.Number) and (self.params.r >= 0)): raise ValueError("parameter r must be a positive number. Was "+str(self.params.r))
    ...
    def prepare_prizes(self, prize_fp):
        """
        ...
        Arguments:
            prize_fp (str or FILE): a filepath or file object containing a tsv **with column headers**.
        """

        prizes_df = pd.read_csv(prize_fp)

        cns = prizes_df.columns.tolist()
        pcns = ['name', 'prize', 'species']
        cns = pcns + [cn for cn in cns if cn not in pcns ]
        prizes_df = prizes_df[cns]

        prizes_df['prize'] = pd.to_numeric(prizes_df['prize'])

        return self._prepare_prizes(prizes_df)


    def _prepare_prizes(self, prizes_df):

        # Some files have duplicated genes with different prizes. Keep the max prize.
        duplicatedNodeNames = prizes_df[prizes_df.set_index('name').index.duplicated()]['name'].tolist()
        if len(duplicatedNodeNames) > 0:
            if self.params.verbose_oi:
                logger.info("Duplicated gene symbols in the prize file (we'll keep the max prize):")
                logger.info(duplicatedNodeNames)
            prizes_df = prizes_df.groupby('name').max().reset_index()

        # Find the indices of the nodes being assigned prizes
        prizes_df.set_index(self.nodes.get_indexer(prizes_df['name']), inplace=True)

        # there will be some nodes in the prize file which we don't have in our interactome
        nodeNamesNotFound = prizes_df[prizes_df.index == -1]['name'].tolist()
        if len(nodeNamesNotFound) > 0:
            if self.params.verbose_oi:
                if len(nodeNamesNotFound) < 10:
                    logger.info("Members of the prize file not present in the interactome:")
                    logger.info(nodeNamesNotFound)
                else:
                    logger.info("Number of members of the prize file not present in the interactome:")
                    logger.info(len(nodeNamesNotFound))
            prizes_df.drop(-1, inplace=True, errors='ignore')

        # All nodes in the prizes dataframe are defined to be terminals.
        prizes_df["terminal"] = True
        prizes_df["type"] = prizes_df.get("type", default="protein")

        
        # Node attributes dataframe for all proteins in self.nodes.
        self.node_attributes = prizes_df.set_index('name').rename_axis(None).reindex(self.nodes)
        self.node_attributes["degree"] = self.node_degrees
        self.node_attributes["prize"].fillna(0, inplace=True)
        self.node_attributes["type"].fillna("X", inplace=True) # the following is filling in an X for all of the non-terminal nodes not defined in the prize file
        self.node_attributes["terminal"].fillna(False, inplace=True)
        
        self.node_attributes["species"].fillna('X', inplace=True) # the following is filling in an X for all of the non-terminal nodes not defined in the prize file
    ...
    ### keeping edge frequency info, then need to add additional method variable edge_df = pd.DataFrame()
    def output_forest_as_networkx(self, vertex_indices, edge_indices):
        ...
        # Replace the edge indices with the actual edges (nodeName1 name, nodeName2 name) by indexing into the interactome
        edges = self.interactome_dataframe.loc[edge_indices]
        
        ### keeping edge frequency info
        # if len(edges) == len(edge_df):
        #     edges['robustness'] = edge_df['robustness']
        #     edges['specificity'] = edge_df['specificity']
                  
        forest = nx.from_pandas_edgelist(edges, 'nodeName1', 'nodeName2', edge_attr=True)
        ...
        #JFG forest has the following columns : source   target    nodeName2   in_solution   cost
        #JFG augmented_forest at this point has the following columns : source   target    cost    in_solution
        
        # JFG forest is based on the given vertex_indices and edge_indices
        # JFG augmented_forest potentially includes edges present in the interactome but were not part of the solution and thus not found in forest


        # Post-processing
        #JFG betweenness(augmented_forest)
        #JFG louvain_clustering(augmented_forest)
        #JFG annotate_graph_nodes(augmented_forest)
    ...
    def randomizations(self, noisy_edges_reps=0, random_terminals_reps=0):
        ...
        if random_terminals_reps > 0:
            ...
            #JFG modified due to behavior of array concatenation with empty entries is deprecated.
            if not robust_vertices.empty and not specific_vertices.empty:
                vertex_indices = pd.concat([robust_vertices, specific_vertices], axis=1).fillna(0)
            else:
                #JFG return empty forest and augmented_forest if problem generating robust and specific vertices
                return nx.empty_graph(0), nx.empty_graph(0)


            #JFG modified due to behavior of array concatenation with empty entries is deprecated.
            if not robust_edges.empty and not specific_edges.empty:
                edge_indices = pd.concat([robust_edges, specific_edges], axis=1).fillna(0)
            else:
                #JFG return empty forest and augmented_forest if problem generating robust and specific edges
                return nx.empty_graph(0), nx.empty_graph(0)
          
        else:
            #JFG Does not compute specificity column but instead fills it with zeros
            
            if not robust_vertices.empty:
                vertex_indices = pd.concat([robust_vertices], axis=1).fillna(0)
                vertex_indices['specificity'] = 0
            else:
                #JFG return empty forest and augmented_forest if problem generating robust and specific vertices
                return nx.empty_graph(0), nx.empty_graph(0)

            if not robust_edges.empty:
                edge_indices = pd.concat([robust_edges], axis=1).fillna(0)
                edge_indices['specificity'] = 0
            else:
                #JFG return empty forest and augmented_forest if problem generating robust and specific edges
                return nx.empty_graph(0), nx.empty_graph(0)


        ### keeping edge frequency info, then need to provide edge_indices
    ...
    def _eval_PCSF_runs(self, params):
        ...
        self._reset_hyperparameters(params=params)
        paramstring = "W_{:04.2f}_B_{:04.2f}_G_{:04.2f}_R_{:04.2f}".format(self.params.w, self.params.b, self.params.g, self.params.r)
    ...
    #JFG added additional method parameters and changed method
    def grid_randomization(self, prize_fp, Ws=[5], Bs=[1], Gs=[3], Rs=[1], noisy_edges_reps=0, random_terminals_reps=0, edge_noise=0.1, seed=1, verbose_oi=False, max_num_cpus=1):
        """
        Macro function which performs grid search or randomizations or both.

        Arguments:
            prize_fp (str): filepath
        ...
        # added set to prevent duplicates
        param_sets = [{'w': w, 'b': b, 'g': g, 'r': r, 'noisy_edge_reps': noisy_edges_reps, 'random_terminals_reps': random_terminals_reps, "edge_noise": edge_noise, "dummy_mode": "terminals", 'seed': seed, 'verbose_oi': verbose_oi} for (w, b, g, r) in set(product(Ws, Bs, Gs, Rs))]

        self.prepare_prizes(prize_fp)

        n_request_cpus = min(max_num_cpus, len(param_sets))
        if n_request_cpus > 1:
            print("number of grid randomizations to process:", len(param_sets))
            print("cpus requested:", n_request_cpus)
            ...
            for params in param_sets:
                paramstring, forest, augmented_forest = self._eval_PCSF_runs(params)
                # append to dictionary
                results = {**results, paramstring: {"forest": forest, "augmented_forest": augmented_forest}}
        
        return results


    def grid_search(self, prize_fp, Ws, Bs, Gs, Rs):
        """
        ...
        return self.grid_randomization(prize_fp, Ws, Bs, Gs, Rs, 0, 0)
...
def louvain_clustering(nxgraph, partition=None, weight='weight', resolution=1.0, randomize=None, random_state=None):
    """
    ...
    Arguments:
        nxgraph (networkx.Graph): a networkx graph, usually the augmented_forest.
    """
    nx.set_node_attributes(nxgraph, {node: {'louvain_clusters':str(cluster)} for node,cluster in community.best_partition(nxgraph, partition=partition, weight=weight, resolution=resolution, randomize=randomize, random_state=random_state).items()})
...
def summarize_grid_search(results, mode, top_n=np.Infinity, graphName = 'augmented_forest'):
    """
    ...
    results = {paramstring: graphs for paramstring, graphs in results.items() if graphs[graphName].number_of_nodes() > 0}

    #JFG if no results return empty data frame
    if results == {}:
        return pd.DataFrame()


    if mode == "membership": # Summarize single-run parameter search
        series = [pd.Series(1, index=graphs[graphName].nodes(), name=paramstring) for paramstring, graphs in results.items()]
    elif mode == "robustness": # Summarize randomized robustness
        series = [get_networkx_graph_as_dataframe_of_nodes(graphs[graphName])["robustness"].rename(paramstring) for paramstring, graphs in results.items()]
    elif mode == "specificity": # Summarize randomized specificity
        series = [get_networkx_graph_as_dataframe_of_nodes(graphs[graphName])["specificity"].rename(paramstring) for paramstring, graphs in results.items()]
    else:
...
def get_robust_subgraph_Robustness(nxgraph, max_size=np.Infinity):
    ...
    if 'specificity' not in node_attributes_df.columns:
        node_attributes_df['specificity'] = 0
    node_attributes_df.sort_values(["robustness", "specificity"], ascending=[False, True], inplace=True)
    return node_attributes_df.iloc[min(max_size,len(node_attributes_df))-1]["robustness"]
    
def get_robust_subgraph_from_randomizations(nxgraph, min_robustness = 0, max_size=np.Infinity, min_component_size=5):
    """
    ...
    node_attributes_df = get_networkx_graph_as_dataframe_of_nodes(nxgraph)
    node_attributes_df = node_attributes_df[node_attributes_df["robustness"] > 0]
    #JFG prefer to use a minimum robustness score instead of a number of nodes as the constraint
    if min_robustness > 0:
...
def filter_graph_by_component_size(nxgraph, min_size=5):
    ...
    #JFG small_components = [g.nodes() for g in nx.connected_component_subgraphs(nxgraph, copy=False) if g.number_of_nodes() < min_size]
    # connected_component_subgraphs was deprecated with version 2.1 of networkx, and removed with version 2.4.
    ...
    small_components = [g.nodes() for g in (nxgraph.subgraph(c) for c in nx.connected_components(nxgraph)) if g.number_of_nodes() < min_size]
...
def get_networkx_graph_as_dataframe_of_edges(nxgraph):
    ...
    #JFG Using pcst-fast 1.0.10 with networkx 3.2.1 and pandas 2.1.3 returns the following  [ 'source', 'target', 'nodeName2', 'cost', 'in_solution']
    edges = nx.to_pandas_edgelist(nxgraph)
    
    ### keeping edge frequency info
    # if(set([ 'source', 'target', 'cost', 'in_solution','robustness','specificity']).issubset(set(edges.columns))):
    #     edges = edges[['source','target','cost','in_solution','robustness','specificity']]
    #     edges.rename(columns = {'source':'nodeName1'}, inplace = True) 
    #     edges.rename(columns = {'target':'nodeName2'}, inplace = True) 
    #     return edges    
        
    if(set([ 'source', 'target', 'cost', 'in_solution']).issubset(set(edges.columns))):
        edges = edges[['source','target','cost','in_solution']]
        edges.rename(columns = {'source':'nodeName1'}, inplace = True) 
        edges.rename(columns = {'target':'nodeName2'}, inplace = True) 
        edges['weight'] = 1.5 - edges['cost']
        edges.loc[edges['weight'] < 0,'weight'] = 0
        return edges
    
    #JFG Original version that uses pcst-fast 1.0.7 with networkx 2.1 and pandas 0.23.4
    return nx.to_pandas_edgelist(nxgraph, 'nodeName1', 'nodeName2')
